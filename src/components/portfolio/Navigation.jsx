  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-[#0A2540]/95 backdrop-blur-md shadow-lg' : 'bg-transparent'
      }`}
    >
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <button
                ...
                className="text-white/90 hover:text-[#00D4FF] font-medium transition-colors duration-300 relative group"
              >
                {item.label}
